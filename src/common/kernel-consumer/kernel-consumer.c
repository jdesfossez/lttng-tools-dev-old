...
/*
 * Check if the local version of the metadata stream match with the version
 * of the metadata stream in the kernel. If it got updated, set the reset flag
 * on the stream.
 */
static
int metadata_stream_check_version(int infd, struct lttng_consumer_stream *stream)
{
	...
	if (ret < 0) {
		ERR("Getting the metadata version");
		goto end;
	}
