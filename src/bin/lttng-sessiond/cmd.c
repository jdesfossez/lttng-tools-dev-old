 ...
 */
static
int check_metadata_regenerate_availability(struct ltt_session *session)
{
	int ret;
...
int ust_metadata_regenerate(struct ltt_ust_session *usess)
{
	...
	cds_list_for_each_entry(uid_reg, &usess->buffer_reg_uid_list, lnode) {
		...
		if (ret) {
			...
			ERR("Failed to generate session metadata (errno = %d)",
					ret);
			goto end;
		}
...
int cmd_metadata_regenerate(struct ltt_session *session)
{
	...
	ret = check_metadata_regenerate_availability(session);
	if (ret != 0) {
		goto end;
	}

	if (session->kernel_session) {
		ret = kernctl_session_metadata_regen(session->kernel_session->fd);
		if (ret < 0) {
			ERR("Regenerating the kernel metadata");
			goto end;
		}
	}
	...
	if (session->ust_session) {
		...
		if (ret < 0) {
			ERR("Regenerating the UST metadata");
			goto end;
		}
	}
